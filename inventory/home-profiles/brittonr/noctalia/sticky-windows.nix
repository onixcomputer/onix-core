...
let
  ...
  stickyDaemon = pkgs.writeShellApplication {
    name = "niri-sticky-daemon";
    runtimeInputs = [
      niriPackage
      pkgs.jq
      pkgs.gnused
    ...
    text = ''
      ...
      ${niri} msg --json event-stream | while IFS= read -r event; do
        ...
        ws_id=$(echo "$event" | ${jq} -r '.WorkspaceActivated // empty | select(.focused == true) | .id // empty' 2>/dev/null) || true
        [ -z "$ws_id" ] && continue
        ...
        ws_idx=$(${niri} msg --json workspaces | ${jq} -r ".[] | select(.id == $ws_id) | .idx") || true
        [ -z "''${ws_idx:-}" ] && continue
  ...
  toggleSticky = pkgs.writeShellApplication {
    name = "toggle-sticky-window";
    runtimeInputs = [
      niriPackage
      pkgs.jq
      pkgs.gnugrep
...
in
{
  ...
  systemd.user.services.niri-sticky-daemon = {
    ...
    Service = {
      Type = "simple";
      ExecStart = lib.getExe stickyDaemon;
      Restart = "on-failure";
