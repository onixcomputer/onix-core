  ...
  home.packages = with pkgs; [
    ...
    (writeShellScriptBin "wallpaper" ''
      ...
      case "$1" in
        clear)
          rm -f "$OVERRIDE_FILE"
          echo "Cleared override config. Using Nix defaults."
          ;;
        show)
          echo "=== Current Wallpaper Configuration ==="
          echo ""
          if [[ -f "$OVERRIDE_FILE" ]]; then
            echo "📌 OVERRIDE ACTIVE (temporary):"
          ...
          echo ""
          if [[ -f "$OVERRIDE_FILE" ]]; then
            echo "Run 'wallpaper clear' to use Nix defaults"
          fi
          ;;
        help)
      ...
      EOF
          echo "Override config updated. Run 'wallpaper show' to see current values."
          echo "This is temporary. Edit themes.nix and rebuild for permanent changes."
          ;;
        *)
          echo "Usage: wallpaper <command> [options]"
          echo ""
          echo "Commands:"
          echo "  show   - Display current configuration values"
          echo "  help   - Show all available options and parameters"
          echo "  set    - Create temporary override for testing"
          echo "  clear  - Remove override, use Nix defaults"
          echo ""
          echo "Set command uses named parameters:"
          echo "  wallpaper set <param>=<value> ..."
          echo ""
          echo "Parameters:"
          ...
          echo ""
          echo "Examples:"
          echo "  wallpaper show                              # Current config"
          echo "  wallpaper help                              # All options"
          echo "  wallpaper set transition=wave duration=5    # Test wave"
          echo "  wallpaper set resize=fit fill=1a1b26        # Dark padding"
          echo "  wallpaper set filter=Nearest                # Pixel art mode"
          echo "  wallpaper clear                             # Reset to Nix"
          echo ""
          echo "Note: 'set' creates temporary overrides. Edit themes.nix for permanent changes."
          ;;
      esac
    '')
    ...
    (writeShellScriptBin "restore-wallpaper" ''
      ...
      # Check if wallpaper file still exists
      if [[ ! -f "$WALLPAPER" ]]; then
    ...
    (writeShellScriptBin "set-wallpaper" ''
      ...
      # Check for override file (for testing without rebuilding)
      OVERRIDE_FILE="$HOME/.config/wallpaper/override"
      if [[ -f "$OVERRIDE_FILE" ]]; then
        source "$OVERRIDE_FILE"
      fi
    ...
    (writeShellScriptBin "rofi-wallpaper" ''
      ...
      done < <(find "$WALLPAPER_DIR" -maxdepth 1 -type f \( -iname "*.mp4" -o -iname "*.webm" -o -iname "*.mkv" \) | sort)
