          ...
          lib.mkMerge [
            {
              ...
              services.searx = {
                enable = true;
                package = if settings.enableKagi then import ./kagi-package.nix { inherit pkgs; } else pkgs.searxng;
                configureUwsgi = true;
                ...
                settings = {
                  ...
                  engines = lib.mkIf settings.enableKagi [
                    {
                      name = "kagi-private";
                      engine = "kagi_session";
                      shortcut = "kg";
              ...
              # Both files are consumed only by the private runtime YAML renderer.
              systemd.services.searx-init.serviceConfig.EnvironmentFile = lib.mkIf settings.enableKagi (
                lib.mkForce [
              ...
              clan.core.vars.generators.${kagiGeneratorName} = lib.mkIf settings.enableKagi {
                files = {
                  "env-file" = {
                    secret = true;
                    mode = "0400";
