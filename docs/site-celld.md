...
- public port: `32110`
- internal port: `32111`
- loopback-only Celld backend: `127.0.0.1:32112`
...
Celld `0.3.0` rejects non-root request paths that end in `/` before asset routing. The `celld-site-ingress` nginx adapter removes only the final slash before it forwards a request to Celld on port `32112`. The public URL stays unchanged. Celld binds its backend to loopback, so Tailnet peers cannot bypass ingress.
