...
self-test)
  ...
  exec "@dataMovementExecutable@" self-test
  ;;
validate-runtime)
  shift
  require_no_suffix_arguments "$mode" "$#"
  ...
  validate_runtime_state
  prepare_runtime_state
  printf 'ttWKV7 data-movement runtime state preflight: PASS\n'
  ;;
probe)
  shift
  require_no_suffix_arguments "$mode" "$#"
  ...
  validate_runtime_state
  prepare_runtime_state
  exec "@dataMovementExecutable@" probe
  ;;
*)
  printf 'wkv7-data-movement: usage: wkv7-data-movement self-test|validate-runtime|probe\n' >&2
