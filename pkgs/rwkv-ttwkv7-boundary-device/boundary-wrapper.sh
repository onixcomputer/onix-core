...
artifact_directory_name="rwkv-boundary-device"
fixture_path="@fixturePath@"
runtime_executable="@runtimeExecutable@"

error_out() {
  printf 'wkv7-rwkv-boundary: %s\n' "$1" >&2
...
self-test)
  ...
  exec "$runtime_executable" boundary-self-test "$fixture_path"
  ;;
validate-runtime)
  shift
  require_no_suffix_arguments "$mode" "$#"
  ...
  validate_runtime_state
  prepare_runtime_state
  "$runtime_executable" boundary-self-test "$fixture_path" >/dev/null
  printf 'rwkv ttWKV7 boundary runtime state preflight: PASS\n'
  ;;
probe)
  shift
  require_no_suffix_arguments "$mode" "$#"
  ...
  validate_runtime_state
  prepare_runtime_state
  exec "$runtime_executable" boundary-run "$fixture_path" \
    "$TT_METAL_LOGS_PATH/$artifact_directory_name"
  ;;
*)
  printf 'wkv7-rwkv-boundary: usage: wkv7-rwkv-boundary self-test|validate-runtime|probe\n' >&2
