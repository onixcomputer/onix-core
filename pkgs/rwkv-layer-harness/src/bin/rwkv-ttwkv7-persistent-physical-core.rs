use rwkv_layer_harness::{
    Ttwkv7ObservedLayerEvidence, begin_ttwkv7_persistent_physical_model_driver,
    emulate_ttwkv7_dispatch_response_frame, validate_ttwkv7_dispatch_response_frame,
};
use std::error::Error;
use std::fs;
...
const NEGATIVE_RESPONSE_OPTION: &str = "--negative-response-self-test";
const EMIT_FIRST_REQUEST_OPTION: &str = "--emit-first-request";
const VALIDATE_RESPONSE_OPTION: &str = "--validate-response";
const EXPECTED_ARGUMENT_COUNT: usize = 3;
const RESPONSE_VALIDATION_ARGUMENT_COUNT: usize = 4;
const EXPECTED_DISPATCH_CALL_COUNT: usize = 24;
const NEGATIVE_RESPONSE_PASS_MARKER: &str =
    "persistent physical core negative response self-test: PASS";
const RESPONSE_VALIDATION_PASS_MARKER: &str = "persistent physical core response validation: PASS";
const USAGE: &str = "usage: rwkv-ttwkv7-persistent-physical-core --evidence-root PATH | --negative-response-self-test PATH | --emit-first-request PATH | --validate-response REQUEST RESPONSE";
...
fn main() -> ExitCode {
    match run() {
        Ok(()) => ExitCode::SUCCESS,
        Err(error) => {
            eprintln!("rwkv-ttwkv7-persistent-physical-core: {error}");
            ExitCode::FAILURE
        }
    }
}
...
fn run() -> Result<(), Box<dyn Error>> {
    ...
    if arguments.len() == RESPONSE_VALIDATION_ARGUMENT_COUNT
        && arguments[1] == VALIDATE_RESPONSE_OPTION
    {
        let request = read_file(Path::new(&arguments[2]))?;
        let response = read_file(Path::new(&arguments[3]))?;
        validate_ttwkv7_dispatch_response_frame(&request, &response).map_err(invalid_data)?;
        println!("{RESPONSE_VALIDATION_PASS_MARKER}");
        return Ok(());
    }
    if arguments.len() != EXPECTED_ARGUMENT_COUNT
        || (arguments[1] != EVIDENCE_ROOT_OPTION
            && arguments[1] != NEGATIVE_RESPONSE_OPTION
            && arguments[1] != EMIT_FIRST_REQUEST_OPTION)
    {
        return Err(io::Error::new(io::ErrorKind::InvalidInput, USAGE).into());
    }
    let negative_response_self_test = arguments[1] == NEGATIVE_RESPONSE_OPTION;
    let emit_first_request = arguments[1] == EMIT_FIRST_REQUEST_OPTION;
    ...
    if emit_first_request {
        let request = driver.prepare_next_request().map_err(invalid_data)?;
        io::stdout().lock().write_all(&request.frame)?;
        return Ok(());
    }
    if negative_response_self_test {
        let request = driver.prepare_next_request().map_err(invalid_data)?;
        let response =
            emulate_ttwkv7_dispatch_response_frame(&request.frame).map_err(invalid_data)?;
        let truncated_length = response
            .len()
            .checked_sub(1)
            .ok_or_else(|| invalid_data("response fixture is unexpectedly empty".to_owned()))?;
        if driver
            .accept_response(&response[..truncated_length])
            .is_ok()
        {
            return Err(invalid_data("truncated response was accepted".to_owned()).into());
        }
        if driver.prepare_next_request().is_ok() {
            return Err(invalid_data("faulted driver accepted another request".to_owned()).into());
        }
        if driver.finish().is_ok() {
            return Err(invalid_data("faulted driver produced a receipt".to_owned()).into());
        }
        println!("{NEGATIVE_RESPONSE_PASS_MARKER}");
        return Ok(());
    }
    for expected_call in 0..EXPECTED_DISPATCH_CALL_COUNT {
        let request = driver.prepare_next_request().map_err(invalid_data)?;
        if request.call_ordinal != expected_call {
            return Err(invalid_data(format!(
                "dispatch call order mismatch: expected {expected_call}, found {}",
        ...
        if progress.accepted_call_count != expected_call + 1 {
            return Err(invalid_data("dispatch progress count mismatch".to_owned()).into());
        }
    }
    let receipt = driver.finish().map_err(invalid_data)?;
