use rwkv_layer_harness::run_ttwkv7_boundary_checkpoint;
use std::error::Error;
use std::fs;
...
fn main() -> ExitCode {
    match run() {
        Ok(()) => ExitCode::SUCCESS,
        Err(error) => {
            eprintln!("rwkv-ttwkv7-fixture: {error}");
            ExitCode::FAILURE
        }
    }
}
...
fn run() -> Result<(), Box<dyn Error>> {
    ...
    if arguments.len() != EXPECTED_ARGUMENT_COUNT {
        return Err(io::Error::new(
            ...
            "rwkv-ttwkv7-fixture does not accept arguments",
        )
        .into());
    }
    ...
    let receipt =
        run_ttwkv7_boundary_checkpoint(&checkpoint, expected_blake3).map_err(invalid_data)?;
