use rwkv_layer_harness::{Ttwkv7ObservedLayerEvidence, run_ttwkv7_observed_model_carry_checkpoint};
use std::error::Error;
use std::fs;
...
fn main() -> ExitCode {
    match run() {
        Ok(()) => ExitCode::SUCCESS,
        Err(error) => {
            eprintln!("rwkv-ttwkv7-observed-model-carry: {error}");
            ExitCode::FAILURE
        }
    }
}
...
fn run() -> Result<(), Box<dyn Error>> {
    ...
    if arguments.len() != EXPECTED_ARGUMENT_COUNT || arguments[1] != EVIDENCE_ROOT_OPTION {
        return Err(io::Error::new(
            ...
            "usage: rwkv-ttwkv7-observed-model-carry --evidence-root PATH",
        )
        .into());
    }
    ...
    let receipt =
        run_ttwkv7_observed_model_carry_checkpoint(&checkpoint, expected_blake3, &evidence)
