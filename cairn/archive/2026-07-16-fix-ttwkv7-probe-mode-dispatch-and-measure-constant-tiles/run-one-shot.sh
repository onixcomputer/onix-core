...
readonly reviewed_base_commit="7145165bc6e31454c2b0eebed6fcf5d48067f30a"
...
readonly package_path="/nix/store/3f485zv9vz38rd8048bwc7qkshg2m5cl-ttwkv7-unstable-2026-06-22"
...
readonly expected_probe_exec_line="exec \"$probe_runtime_path\" probe \"\$@\""
...
readonly run_root="/var/tmp/ttwkv7-constant-probe-20260716T185657Z"
...
[[ -x $probe_runtime_path ]] || fail "reviewed probe runtime is not executable: $probe_runtime_path"
...
grep -F -- "$expected_probe_exec_line" "$probe_path" >"$run_root/production-wrapper-target-at-execution.txt" ||
  fail "production probe wrapper target mismatch"
if grep -F -- "$unsafe_probe_exec_prefix" "$probe_path" >"$run_root/production-wrapper-unsafe-target-at-execution.txt"; then
  fail "production probe wrapper retained runtime out expansion"
fi
...
arm_rollback_timer
printf '%s\n' "1" >"$run_root/rollback-arm-attempted.txt"
owner_isolation_attempted=1
printf '%s\n' "1" >"$run_root/service-stop-attempted.txt"
...
[[ $ownership_status -eq 1 && ! -s $run_root/device-owner-after-stop.txt ]] ||
