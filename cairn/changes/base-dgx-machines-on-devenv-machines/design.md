...
### 5. Use SOPS for steady-state secrets

**Choice:** Devenv-owned DGX systems will use `sops-nix` for steady-state secrets. Service units will read runtime credential files.

No secret value can enter Nickel, generated inventory data, Nix options, command arguments, logs, or the Nix store.

The future bootstrap change can use the pull request's SecretSpec support to copy the minimum decryption identity. That change will define the bootstrap trust process.

**Rationale:** The pull request uses SecretSpec for installation files. Its documentation recommends a NixOS secret manager for steady-state services.
