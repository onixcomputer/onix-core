...
readonly wrapper_path="$persistent_package_path/bin/rwkv-ttwkv7-persistent-device"
readonly plan_manifest_path="$persistent_package_path/share/rwkv-ttwkv7-persistent-device/session/manifest.json"
readonly plan_receipt_path="$persistent_package_path/share/rwkv-ttwkv7-persistent-device/session/plan-receipt.json"
...
readonly expected_process_count="1"
readonly expected_response_connection_count="1"
readonly expected_retry_count="0"
readonly expected_comparison_pass_count="24"
readonly expected_generated_token_record_count="4"
readonly expected_generated_token_id="2"
...
fail() {
  local diagnostic="$1"
  if [[ -d $run_root ]]; then
    printf 'rwkv persistent physical ttWKV7 dispatch: %s\n' "$diagnostic" >"$run_root/preflight-failure.txt" || true
  fi
  printf 'rwkv persistent physical ttWKV7 dispatch: %s\n' "$diagnostic" >&2
...
validate_manifest_entry() {
  ...
  if [[ $expected_bytes != "any" ]]; then
    [[ $actual_bytes -eq $expected_bytes ]]
  fi
  actual_blake3="$($b3sum_path "$file_path" | cut -d' ' -f1)"
  [[ $(grep -Fxc "$role"$'\t'"$filename"$'\t'"$actual_bytes"$'\t'"$actual_blake3" "$artifact_root/manifest.tsv") -eq 1 ]]
}

validate_persistent_artifacts() (
  set -e
  [[ -s $run_root/process-receipt.json ]]
  grep -Fq '"wrapper_invocation_count": '"$expected_process_count" "$run_root/process-receipt.json"
  grep -Fq '"metalium_child_process_budget": '"$expected_process_count" "$run_root/process-receipt.json"
  grep -Fq '"response_connection_budget": '"$expected_response_connection_count" "$run_root/process-receipt.json"
  grep -Fq '"physical_call_budget": '"$expected_physical_call_count" "$run_root/process-receipt.json"
  grep -Fq '"retry_count": '"$expected_retry_count" "$run_root/process-receipt.json"
  grep -Fq '"reconnect_count": '"$expected_retry_count" "$run_root/process-receipt.json"
  [[ -s $artifact_root/core-receipt.json ]]
  [[ -s $artifact_root/server-summary.json ]]
  [[ -f $artifact_root/server-stdout.log ]]
  [[ -f $artifact_root/server-stderr.log ]]
  [[ -s $artifact_root/transcript.bin ]]
  ...
  [[ $(head -n 1 "$artifact_root/manifest.tsv") == $'role\tfilename\tbytes\tblake3' ]]
  validate_manifest_entry "host_receipt" "receipt.json" "any"
  validate_manifest_entry "core_receipt" "core-receipt.json" "any"
  validate_manifest_entry "server_summary" "server-summary.json" "any"
  validate_manifest_entry "transcript" "transcript.bin" "$expected_transcript_bytes"
  validate_manifest_entry "server_stdout" "server-stdout.log" "any"
  validate_manifest_entry "server_stderr" "server-stderr.log" "any"
  grep -Fq '"target": "rwkv_ttwkv7_persistent_physical_dispatch"' "$artifact_root/receipt.json"
  grep -Fq '"device_open_count": '"$expected_process_count" "$artifact_root/receipt.json"
  grep -Fq '"workload_enqueue_count": '"$expected_physical_call_count" "$artifact_root/receipt.json"
  grep -Fq '"physical_wkv_call_count": '"$expected_physical_call_count" "$artifact_root/receipt.json"
  grep -Fq '"metalium_child_process_count": '"$expected_process_count" "$artifact_root/receipt.json"
  grep -Fq '"response_channel": "unix_stream"' "$artifact_root/receipt.json"
  grep -Fq '"response_connection_count": '"$expected_response_connection_count" "$artifact_root/receipt.json"
  grep -Fq '"retry_count": '"$expected_retry_count" "$artifact_root/receipt.json"
  grep -Fq '"reconnect_count": '"$expected_retry_count" "$artifact_root/receipt.json"
  grep -Fq '"terminal_state": "closed"' "$artifact_root/receipt.json"
  grep -Fq '"session_call_count": '"$expected_physical_call_count" "$artifact_root/receipt.json"
  grep -Fq '"session_continuity_check_count": '"$expected_continuity_count" "$artifact_root/receipt.json"
  grep -Fq '"selected_fourth_token_id": '"$expected_generated_token_id" "$artifact_root/receipt.json"
  grep -Fq '"target": "rwkv_ttwkv7_persistent_physical_core"' "$artifact_root/core-receipt.json"
  grep -Fq '"physical_wkv_call_count": '"$expected_physical_call_count" "$artifact_root/core-receipt.json"
  grep -Fq '"new_physical_wkv_call_count": '"$expected_physical_call_count" "$artifact_root/core-receipt.json"
  grep -Fq '"session_call_count": '"$expected_physical_call_count" "$artifact_root/core-receipt.json"
  grep -Fq '"session_continuity_check_count": '"$expected_continuity_count" "$artifact_root/core-receipt.json"
  grep -Fq '"selected_fourth_token_id": '"$expected_generated_token_id" "$artifact_root/core-receipt.json"
  [[ $(grep -Fc '"passed": true' "$artifact_root/core-receipt.json") -eq $expected_comparison_pass_count ]]
  [[ $(grep -Fc '"generated_token_id": '"$expected_generated_token_id" "$artifact_root/core-receipt.json") -eq $expected_generated_token_record_count ]]
  grep -Fq '"target":"rwkv_ttwkv7_persistent_dispatch_server"' "$artifact_root/server-summary.json"
  grep -Fq '"device_open_count":'"$expected_process_count" "$artifact_root/server-summary.json"
  grep -Fq '"call_count":'"$expected_physical_call_count" "$artifact_root/server-summary.json"
  grep -Fq '"workload_enqueue_count":'"$expected_physical_call_count" "$artifact_root/server-summary.json"
  grep -Fq '"same_layer_state_continuity_count":'"$expected_continuity_count" "$artifact_root/server-summary.json"
  grep -Fq '"response_channel":"unix_stream"' "$artifact_root/server-summary.json"
  grep -Fq '"response_connection_count":'"$expected_response_connection_count" "$artifact_root/server-summary.json"
  grep -Fq '"terminal_state":"closed"' "$artifact_root/server-summary.json"
  if grep -Fq 'test_only_cpu_server' "$artifact_root/server-summary.json"; then
    return "$generic_failure_status"
  fi
...
materialize_classification() {
  ...
  append_artifact "child_stderr" "$artifact_root/server-stderr.log"
  append_artifact "core_receipt" "$artifact_root/core-receipt.json"
  append_artifact "diagnostic_log" "$run_root/diagnostic.log"
  append_artifact "host_manifest" "$artifact_root/manifest.tsv"
  append_artifact "host_receipt" "$artifact_root/receipt.json"
  append_artifact "owner_after" "$run_root/owner-after.properties"
  append_artifact "process_receipt" "$run_root/process-receipt.json"
  append_artifact "server_summary" "$artifact_root/server-summary.json"
  append_artifact "transcript" "$artifact_root/transcript.bin"
...
# r[impl onix.tenstorrent.native_runtime.rwkv_lab.ttwkv7_persistent_metalium_device_4_runbook]
...
[[ $((restoration_health_attempts * restoration_health_delay_seconds)) -eq $restoration_health_window_seconds ]] || fail "restoration health window mismatch"
[[ $($b3sum_path "$plan_manifest_path" | cut -d' ' -f1) == "$expected_plan_manifest_blake3" ]] || fail "plan manifest identity mismatch"
[[ $($b3sum_path "$plan_receipt_path" | cut -d' ' -f1) == "$expected_plan_receipt_blake3" ]] || fail "plan receipt identity mismatch"
[[ $($b3sum_path "$readiness_path/not-run-receipt.json" | cut -d' ' -f1) == "$expected_not_run_blake3" ]] || fail "not-run receipt identity mismatch"
[[ $($b3sum_path "$readiness_path/receipt.json" | cut -d' ' -f1) == "$expected_readiness_blake3" ]] || fail "readiness receipt identity mismatch"
grep -Fq "host_executable=\"$harness_package_path/bin/rwkv-ttwkv7-persistent-physical-dispatch\"" "$wrapper_path" || fail "host executable authority mismatch"
grep -Fq "metalium_server_executable=\"$ordinary_package_path/bin/wkv7\"" "$wrapper_path" || fail "Metalium executable authority mismatch"
...
grep -Fq "\"package_path\": \"$persistent_package_path\"" "$plan_manifest_path" || fail "plan package mismatch"
grep -Fq "\"executable\": \"$wrapper_path\"" "$plan_manifest_path" || fail "plan executable mismatch"
...
grep -Fq "\"inspector_address\": \"$inspector_address\"" "$plan_manifest_path" || fail "plan Inspector mismatch"
grep -Fq '"child_stdout"' "$plan_manifest_path" || fail "plan stdout evidence role mismatch"
grep -Fq "\"max_processes\": $expected_process_count" "$plan_manifest_path" || fail "plan process budget mismatch"
grep -Fq "\"timeout_seconds\": $process_timeout_seconds" "$plan_manifest_path" || fail "plan timeout mismatch"
grep -Fq "\"rollback_delay_seconds\": $rollback_delay_seconds" "$plan_manifest_path" || fail "plan rollback mismatch"
...
printf '%s\n' \
  ...
  '  "schema_version": 1,' \
  "  \"plan_id\": \"$expected_plan_id\"," \
  "  \"wrapper_invocation_count\": $process_attempted," \
  "  \"metalium_child_process_budget\": $expected_process_count," \
  "  \"response_connection_budget\": $expected_response_connection_count," \
  "  \"physical_call_budget\": $expected_physical_call_count," \
  "  \"exit_status\": $process_status," \
  "  \"timed_out\": $process_timed_out," \
  '  "retry_count": 0,' \
  '  "reconnect_count": 0' \
  '}' \
  >"$run_root/process-receipt.json"
set +e
