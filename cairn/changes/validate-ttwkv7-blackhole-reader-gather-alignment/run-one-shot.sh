...
readonly diagnostic_path="$package_path/bin/wkv7-data-movement"
readonly diagnostic_runtime_path="$package_path/libexec/ttwkv7/wkv7-data-movement-runtime"
readonly expected_self_test_exec_line="  exec \"$diagnostic_runtime_path\" self-test"
readonly expected_probe_exec_line="  exec \"$diagnostic_runtime_path\" probe"
...
readonly artifact_root="$logs_path/ttwkv7-data-movement"
readonly manifest_path="$artifact_root/manifest.tsv"
...
readonly expected_probe_exec_line_count="1"
readonly expected_self_test_exec_line_count="1"
readonly expected_total_exec_line_count="2"
readonly expected_device_owner_absence_status="1"
readonly expected_record_count="13"
readonly expected_raw_file_count="13"
readonly expected_argument_file_count="26"
readonly expected_manifest_entry_count="52"
readonly expected_manifest_line_count="53"
readonly expected_aggregate_count="1"
...
readonly -a expected_record_names=(
  cb21-loopback
  input-upload-0
  input-upload-1
  input-upload-2
...
readonly -a expected_log_records=(
  "case=cb21-loopback phase=control "
  "case=input-upload-0 phase=control "
  "case=input-upload-1 phase=control "
  "case=input-upload-2 phase=control "
  "case=input-upload-3 phase=control "
  "case=input-upload-4 phase=control "
  "case=input-upload-5 phase=control "
  "case=state-upload phase=control "
  "case=decode-L1 phase=reader-capture "
  "case=chunked-partial-L1 phase=reader-capture "
  "case=chunked-full-L32 phase=reader-capture "
  "case=chunked phase=writer-scatter "
  "case=decodeL phase=writer-scatter "
)
...
validate_completed_evidence() (
  ...
  [[ ${#expected_record_names[@]} -eq $expected_record_count ]]
  [[ ${#expected_log_records[@]} -eq $expected_record_count ]]
  [[ -f $manifest_path && -r $manifest_path ]]

  manifest_line_count="$(wc -l <"$manifest_path")"
  manifest_entry_count="$((manifest_line_count - 1))"
  [[ $manifest_line_count -eq $expected_manifest_line_count ]]
  [[ $manifest_entry_count -eq $expected_manifest_entry_count ]]

  raw_file_count="$(find "$artifact_root" -maxdepth 1 -type f -name '*.bf16' | wc -l)"
  argument_file_count="$(find "$artifact_root" -maxdepth 1 -type f -name '*.args' | wc -l)"
  [[ $raw_file_count -eq $expected_raw_file_count ]]
  [[ $argument_file_count -eq $expected_argument_file_count ]]

  for name in "${expected_record_names[@]}"; do
    [[ -s $artifact_root/$name.bf16 ]]
    [[ -s $artifact_root/$name-producer.args ]]
    [[ -s $artifact_root/$name-consumer.args ]]
    [[ $(grep -Fc $'capture\t'"$name"$'\t'"$name.bf16"$'\t' "$manifest_path") -eq 1 ]]
    [[ $(grep -Fc $'args\t'"$name-producer"$'\t'"$name-producer.args"$'\t' "$manifest_path") -eq 1 ]]
    [[ $(grep -Fc $'args\t'"$name-consumer"$'\t'"$name-consumer.args"$'\t' "$manifest_path") -eq 1 ]]
    [[ $(grep -Fc $'result\t'"$name"$'\t-\t0\t' "$manifest_path") -eq 1 ]]
  done

  [[ $(grep -c '^case=' "$run_root/diagnostic.log") -eq $expected_record_count ]]
  for log_record in "${expected_log_records[@]}"; do
    [[ $(grep -Fc "$log_record" "$run_root/diagnostic.log") -eq 1 ]]
  done
  aggregate_count="$(grep -Fxc 'data-movement device probe: PASS' "$run_root/diagnostic.log" || true)"
  aggregate_count="$((aggregate_count + $(grep -Fxc 'data-movement device probe: FAIL' "$run_root/diagnostic.log" || true)))"
  [[ $aggregate_count -eq $expected_aggregate_count ]]
)
...
fail() {
  printf 'ttWKV7 aligned-reader validation one-shot: %s\n' "$1" >&2
...
[[ $(grep -Fxc -- "$expected_probe_exec_line" "$diagnostic_path") -eq $expected_probe_exec_line_count ]] ||
  fail "production diagnostic wrapper probe target or vector mismatch"
[[ $(grep -Fxc -- "$expected_self_test_exec_line" "$diagnostic_path") -eq $expected_self_test_exec_line_count ]] ||
  fail "production diagnostic wrapper self-test target or vector mismatch"
[[ $(grep -Ec '^[[:space:]]*exec ' "$diagnostic_path") -eq $expected_total_exec_line_count ]] ||
...
printf '%s\n' "$expected_probe_exec_line" >"$run_root/production-wrapper-target-at-execution.txt"
...
readonly evidence_completeness_status="$?"
set -e
