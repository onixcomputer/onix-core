...
## Purpose
Define the declarative `sccache`-backed Rust build cache behavior for `britton-desktop`, including managed Cargo ownership, local-only cache configuration, fail-open rustc-wrapper behavior, and workstation validation workflows.
## Requirements
