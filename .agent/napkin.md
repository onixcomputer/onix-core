...
|------|--------|----------------|-------------------|
| 2026-05-27 | self | Treated `clan m u bonsai --upload-inputs` `sized: unexpected end-of-file` as a remote copy problem first, but the real cause was local fixed-output flake input store-path corruption; `ssh://` hid the `hash mismatch importing path` that `ssh-ng://` showed | Verify the named source path locally with `nix-store --verify-path`; if local root repair is unavailable, delete invalid partial paths on the target with `nix-store --delete --ignore-liveness` and `nix flake prefetch` the locked input on the target so `ssh://` upload skips the corrupt stream |
