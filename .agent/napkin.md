...
| 2026-03-22 | self | cloud-hypervisor guest VM sends zero packets on TAP interface despite booting to multi-user with all services green | Root cause TBD via serial diagnostics. Research findings: (1) microvm.nix always delete+recreate TAP, we only create-if-missing — stale TAP can have wrong flags. (2) microvm.nix tests use static IPs, never networkd DHCP — so DHCP over virtio-net/TAP is comparatively untested. (3) Missing ConfigureWithoutCarrier and ActivationPolicy=up in .network file — networkd may not bring interface UP if carrier timing is off. (4) dnsmasq bind-dynamic might not detect TAP fast enough — need SIGHUP after TAP creation. (5) Name=ens* match is fragile — use Driver=virtio_net instead. |
