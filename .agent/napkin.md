...
## Domain Notes (clan-core exports system)
- Upstream clan-core exports are scope-keyed: `"serviceName:instanceName:roleName:machineName"` (colon-separated, built by `clanLib.buildScopeKey`).
- perInstance can SET exports via `exports.serviceEndpoints.foo = { ... }` — these are option values collected by the framework.
- perInstance receives `exports` specialArg — the aggregated exports from ALL services, keyed by scope strings.
- To READ exports: use `clanLib.selectExports` or `clanLib.getExport` with scope filters. The old `exports.instances or {}` pattern was dead code (no `instances` attribute exists).
- The exports feature is still marked "experimental" by clan-core.
- For co-located services (same machine), explicit localhost defaults work fine — no need for cross-service discovery.
