...
readonly inspector_address="127.0.0.1:43157"
readonly inspector_port="43157"
...
readonly rollback_unit="rwkv-ttwkv7-persistent-rollback-device-3"
...
validate_persistent_artifacts() (
  ...
  grep -Fq '"target":"rwkv_ttwkv7_persistent_metalium_server"' "$artifact_root/server-summary.json"
  grep -Fq '"device_id":'"$visible_device" "$artifact_root/server-summary.json"
  grep -Fq '"device_open_count":'"$expected_process_count" "$artifact_root/server-summary.json"
  grep -Fq '"request_count":'"$expected_physical_call_count" "$artifact_root/server-summary.json"
  grep -Fq '"response_count":'"$expected_physical_call_count" "$artifact_root/server-summary.json"
  grep -Fq '"workload_enqueue_count":'"$expected_physical_call_count" "$artifact_root/server-summary.json"
  grep -Fq '"continuity_check_count":'"$expected_continuity_count" "$artifact_root/server-summary.json"
  grep -Fq '"closed_cleanly":true' "$artifact_root/server-summary.json"
...
grep -Fq '"max_processes": 1' "$plan_manifest_path" || fail "plan process budget mismatch"
grep -Fq '"timeout_seconds": 1800' "$plan_manifest_path" || fail "plan timeout mismatch"
grep -Fq '"rollback_delay_seconds": 2100' "$plan_manifest_path" || fail "plan rollback mismatch"
