...
`onix-core` owns concrete machine inventory, service packaging, deployment, secrets, firewall policy, monitoring, and backup operations. `aspen1` is the selected bootstrap machine, reached for deployment as `root@aspen1.local`. It must host the first least-authority Radicle node before the cross-repository forge pilot can publish source or change dependency transport.
...
- Add typed Nickel configuration and Nix service lowering for `aspen1`, persistent storage, selective seeding, listener exposure, HTTPS Git access, monitoring, retention, off-host backup, and restore policy.
...
- **Security**: the node holds only its own persistent node identity and selected repository data; its systemd boundary cannot read Aspen1's existing Buildbot, Nix-signing, deployment, or service credentials, and delegate/CI authority remains elsewhere.
