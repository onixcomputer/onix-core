...
**Choice:** Run an instance-specific nginx adapter on the public Site port. Strip only the final slash before proxying to Celld on a loopback-only backend port. Configure Celld asset routing to use `drop-trailing-slash`.
