...
### Requirement: Steady-state secrets stay outside Nix data

r[onix.dgx_devenv.secrets] DGX services MUST read steady-state secrets from runtime SOPS credential files.

#### Scenario: A service uses a secret

r[onix.dgx_devenv.secrets.runtime]
- GIVEN a DGX service needs a Tailscale or Mesh-LLM secret
- WHEN the NixOS service starts
- THEN systemd supplies a runtime credential file
- AND no secret value occurs in Nickel, generated inventory data, Nix options, command arguments, or store paths

#### Scenario: A required runtime secret is absent

r[onix.dgx_devenv.secrets.missing]
- GIVEN a service declares a required runtime secret name without an installed secret
- WHEN the service starts
- THEN the service fails before it joins a network
- AND the log identifies the missing credential name without its value
