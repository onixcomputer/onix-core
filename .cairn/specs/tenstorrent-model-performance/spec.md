...
r[onix.tenstorrent.model_performance.managed_benchmark] `britton-desktop` SHALL expose a manually invoked, Nix-managed VibeThinker Metalium benchmark that MUST compare physical device 0, physical device 1, and the reported P150x2 `1x2` mesh with fixed inputs and bounded execution, MUST isolate mutable artifacts outside the source repository, and MUST restore Qwen only when Qwen was active before the benchmark.

#### Scenario: Active Qwen service is benchmarked and restored

- GIVEN Qwen is active and the retained Q8_0 benchmark model is present
- WHEN an authorized operator starts the managed benchmark
- THEN the runner stops Qwen before acquiring either P150
- AND it runs the fixed device-0, device-1, and reported `1x2` matrix
- AND it writes validated results under its systemd-managed state directory
- AND it restarts Qwen after the matrix completes

#### Scenario: Qwen was already inactive

- GIVEN Qwen is inactive before the benchmark starts
- WHEN the managed matrix completes
- THEN the runner leaves Qwen inactive
- AND it does not misrepresent Qwen as restored

#### Scenario: Benchmark output is invalid or execution fails

- GIVEN Qwen was active before the benchmark
...
- AND its cleanup path restarts Qwen
