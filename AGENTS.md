...
## Clankers
- `self.packages.<system>.clanker-router` is intentionally patched over the upstream flake package. Use it when you need multiple remote OpenAI-compatible backends; the stock binary only handles one custom `--api-base` endpoint at a time. When the upstream `src/bin/clanker_router/main.rs` moves, rebase `patches/clanker-router-local-providers.patch` or `nix build .#nixosConfigurations.aspen2.config.system.build.toplevel` will fail during the router package patch phase.
